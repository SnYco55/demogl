...
function isMemberActiveOnDate(member: Member, date: string): boolean {
  if (!date) return true
...
const filteredMembers = computed(() => {
  return members.value.filter((member) => isMemberActiveOnDate(member, selectedDate.value))
})
    ...
    <section class="mx-auto w-full max-w-3xl space-y-4">
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <label for="member-date" class="block text-sm font-medium text-gray-700">
      ...
      <div v-else class="space-y-3">
        <MemberListItem v-for="member in filteredMembers" :key="member.id" :member="member" />
      </div>
    </section>
  </main>
</template>
